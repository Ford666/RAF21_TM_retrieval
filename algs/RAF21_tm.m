function [A_pr, ts, errCurve, corrCurve] = RAF21_tm(y, x, opts)
    ...
    if useGPU
        ...
        for i = 1:nParallel:m
            ...
            while t <= iters   
                ...
                if t<= ceil(ratio*iters)
                    grad = Xinvfun( (curr_y - ysub.^2 .* curr_y./abs(curr_y)) .* weight) / p;
                else
        ...
        while t <= iters   
            ...
            if t<= ceil(ratio*iters)
                grad = Xinvfun( (curr_y - ymag.^2 .* curr_y./abs(curr_y)) .* weight) / p;
            else
